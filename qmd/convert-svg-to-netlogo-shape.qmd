...
This document convert a SVG file into a [NetLogo](https://www.netlogo.org/) [XML](https://en.wikipedia.org/wiki/XML) shape (`.nlogox`). Each shape is extracted and saved in a designated directory.
...
```{r}
svg_file <- here("svg", "netlogo-7-0-3", "acorn.svg")
```

### Set SVG Directory

```{r}
xml_dir <- here("xml", "test")
```
...
::: {.callout-note}
Generated with [Posit](https://posit.co/) [Databot](https://positron.posit.co/databot.html) and [Claude Opus 4.5](https://claude.ai/).
...
hex_to_xml <- setNames(netlogo_xml_colors, toupper(netlogo_hex_colors))
```
...
convert_multiple_svg_to_netlogo <- function(svg_files, output_dir = xml_dir) {
  xml_lines <- c(
  ...
  c(xml_lines, '</turtleShapes>') |>
    paste(collapse = "\n") |>
    write_lines(
      svg_files |>
        basename() |>
        str_replace("\\.svg$", ".xml") |>
        file.path(output_dir, x = _)
    )
...
```{r}
svg_file |> convert_multiple_svg_to_netlogo()
```
